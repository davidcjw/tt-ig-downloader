...
A small, ergonomic command-line tool to download TikTok videos — **single or batch** — built on top of [`yt-dlp`](https://github.com/yt-dlp/yt-dlp), the most reliable TikTok engine.

<p align="center">
  <img src="docs/demo.gif" alt="ttdl downloading a TikTok video from the command line" width="700">
</p>
...
It wraps `yt-dlp` with TikTok-tuned defaults so you don't have to remember flags:

- ✅ **Watermark-free** downloads (yt-dlp's default TikTok format)
...
TikTok changes its page structure often, which breaks older extractors. This tool **prefers a fresh `yt-dlp` from your active Python environment** and pulls in browser **impersonation** (`curl_cffi`) for reliability. An out-of-date `yt-dlp` will fail with *"Unable to extract webpage video data"* — keep it current (`pip install -U "yt-dlp[default]"`).
...
ttdl url1 url2 url3 -o ~/Videos/tiktok -j 4
...
| Flag | Description |
|------|-------------|
| `urls...` | One or more TikTok video / profile URLs |
...
| `--allow-any` | Allow non-TikTok URLs (yt-dlp supports many sites) |
...
Download only content you have the right to. Respect TikTok's Terms of Service and creators' copyright. This tool is for personal/archival use; you are responsible for how you use it.
